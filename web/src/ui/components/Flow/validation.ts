...
export const allowsConnection = (a: NodeKind, b: NodeKind) => NodeConstraints[a].canConnectTo.includes(b)
...
export function isValidConnection(conn: Connection, ns: Node<NodeData>[], es: Edge[]) {
  ...
  if (targetHandleId) {
    const existingOnHandle = es.some(e => e.target === tgt.id && e.targetHandle === targetHandleId)
...
export function validatePipeline(ns: Node<NodeData>[], es: Edge[]): string[] | null {
  if (ns.length === 0) {
